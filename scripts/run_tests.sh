	...
	kubectl get pod/test-1 -n boo -o yaml | grep priority -B1 -A1
	
    echo
	echo "Test 2 - Checking that a Deployment gets scheduled with a priorityClassName mutated by webhook"
	kubectl get deployment/test-2 -n boo -o yaml | grep priority -B1 -A1
	...
	kubectl get deployment/test-3 -n boo -o yaml | grep priority -B1 -A1